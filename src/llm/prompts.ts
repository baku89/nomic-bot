...
- propose_game_end: ゲームが終了している可能性をユーザが指摘 (例: 「勝者出てない?」「もう勝ったよね?」) したとき、または現在のルールから明らかに終了条件が読み取れるとき。
  - reason: 終了が妥当な理由を自然言語で記述する。勝者が特定できるなら reason の中に「@Foo の勝利 (Rule X による)」のように含める。勝者がいない/曖昧な場合はそのまま理由だけ書く
