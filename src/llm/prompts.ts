...
- close_voting_now: 進行中の提案について、参加者が「投票を締め切ってほしい」「集計してほしい」「もう打ち切ろう」等を依頼したとき
  - reason: 締切依頼の文脈 (例: 「期限切れ」「全員投票済み」「進行加速のため」)。**期限未到来でも実行可** (即時に現在の投票で集計される)
  - 「進行中の提案」が無いチャンネルでこのアクションを返してはいけない。その場合は noop を返すこと
